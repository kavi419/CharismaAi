...
public class ReportsController {
        ...
        public String reports(Model model) {
                ...
                // Calculate Smile and Posture Averages
                double averageSmileScore = 0.0;
                double averagePostureScore = 0.0;
                if (!sessions.isEmpty()) {
                        double totalSmile = sessions.stream().mapToDouble(PracticeSession::getSmileScore).sum();
                        double totalPosture = sessions.stream().mapToDouble(PracticeSession::getPostureScore).sum();

                        averageSmileScore = Math.round((totalSmile / sessions.size()) * 10.0) / 10.0;
                        averagePostureScore = Math.round((totalPosture / sessions.size()) * 10.0) / 10.0;
                }
                ...
                model.addAttribute("averageSmileScore", averageSmileScore);
                model.addAttribute("averagePostureScore", averagePostureScore);
